...
// Rule-based anti-fraud checks for Stars credit (deposits / manual credit).
// Best-effort: if Redis is not configured, we allow credit (Install Wizard).
export async function evaluateStarsCreditRisk(params: {
  userId: string;
  stars: number;
  scope: "DEPOSIT" | "MANUAL";
}): Promise<StarsRiskResult> {
  ...
  const maxPerDay = Number(env.STARS_RISK_MAX_CREDIT_PER_USER_PER_DAY || 200000);
  const maxPerHour = Number(env.STARS_RISK_MAX_CREDITS_PER_USER_PER_HOUR || 8);
  const minGapSec = Number(env.STARS_RISK_MIN_SECONDS_BETWEEN_CREDITS || 20);
