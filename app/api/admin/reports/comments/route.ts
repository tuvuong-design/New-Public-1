...
export async function POST(req: Request) {
  ...
  if (ct.includes("application/json")) body = schema.parse(await req.json());
  else {
    const form = await req.formData();
    body = schema.parse({ id: String(form.get("id") || ""), status: String(form.get("status") || "") });
  }
  ...
  if (!ct.includes("application/json")) {
    const back = req.headers.get("referer") || "/admin/reports/comments";
