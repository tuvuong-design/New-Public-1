...
export default async function TagPage({ params, searchParams }: { params: { slug: string }; searchParams: { page?: string } }) {
  ...
  return (
    <main className="space-y-4">
      <div className="card">
        <div className="text-lg font-extrabold">#{tag.name}</div>
        <div className="small muted mt-1">Browse by tag (Task 9).</div>
        <div className="mt-3 flex flex-wrap gap-2">
          <a className="btn" href="/explore">Explore</a>
          <a className="btn" href={`/search?tag=${encodeURIComponent(tag.slug)}`}>Search in tag</a>
        </div>
      </div>
      ...
      <div className="row" style={{ gap: 10, justifyContent: "center" }}>
        {page > 1 ? <a className="btn" href={`/tag/${tag.slug}?page=${page - 1}`}>Prev</a> : null}
        {page < totalPages ? <a className="btn" href={`/tag/${tag.slug}?page=${page + 1}`}>Next</a> : null}
      </div>
    </main>
  );
}
